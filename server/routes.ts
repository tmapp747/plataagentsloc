...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/generate-welcome-video', async (req: Request, res: Response) => {
    try {
      const { applicationId, template = 'welcome' } = req.body;
      ...
      // Generate personalized welcome video
      const videoUrl = await personalizedVideoService.createPersonalizedVideo(application, template);
      
      res.json({ 
        message: 'Personalized welcome video generated successfully',
        videoUrl,
        embedCode: personalizedVideoService.getVideoEmbedCode(videoUrl, false)
      });

    } catch (error) {
      console.error('Error generating personalized video:', error);
      res.status(500).json({ error: 'Failed to generate personalized video' });
    }
  });

  // Generate complete welcome video suite
  app.post('/api/generate-welcome-video-suite', async (req: Request, res: Response) => {
    try {
      ...
      const videos = await personalizedVideoService.generateWelcomeVideoSuite(application);
      
      // Add embed codes for each video
      const result = {
        videos,
        embedCodes: {
          welcome: personalizedVideoService.getVideoEmbedCode(videos.welcomeVideo, true),
          package: videos.packageVideo ? personalizedVideoService.getVideoEmbedCode(videos.packageVideo, false) : null,
          nextSteps: videos.nextStepsVideo ? personalizedVideoService.getVideoEmbedCode(videos.nextStepsVideo, false) : null
        }
      };
      
      res.json({ 
        message: 'Welcome video suite generated successfully',
        ...result
      });

    } catch (error) {
      console.error('Error generating welcome video suite:', error);
      res.status(500).json({ error: 'Failed to generate welcome video suite' });
    }
  });

  // Get video script for preview
  app.get('/api/video-script/:applicationId/:template', async (req: Request, res: Response) => {
    try {
      ...
      const script = await personalizedVideoService.generateVideoScript(application, template as any);
